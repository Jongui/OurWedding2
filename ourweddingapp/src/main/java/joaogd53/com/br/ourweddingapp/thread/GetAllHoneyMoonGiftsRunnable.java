 ...
 */
public class GetAllHoneyMoonGiftsRunnable extends AbstractConnection {
    public GetAllHoneyMoonGiftsRunnable(Activity context) {
        super(context);
    }
        ...
        try {
            ...
            if (this.returnCode == 0) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    HoneyMoonGift.HoneyMoonGiftBuilder.buildFromJson(giftJson);
                }
            } else {
    ...
    public void run() {
        try {
            URLConnection connection = ConnectionFactory.getInstance()
                    .connectionFactory(ConnectionFactory.GET_ALL_HONEY_MOON_GIFTS_SERVLET);
