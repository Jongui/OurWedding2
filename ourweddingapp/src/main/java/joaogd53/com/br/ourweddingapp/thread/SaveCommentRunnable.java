...
public class SaveCommentRunnable extends AbstractConnection {
    private StoryComment comment;

    public SaveCommentRunnable(Activity context, StoryComment comment) {
        super(context);
    ...
    public void run() {
        try {
            URLConnection connection = ConnectionFactory.getInstance()
                    .connectionFactory(ConnectionFactory.SAVE_COMMENT_RUNNABLE);
